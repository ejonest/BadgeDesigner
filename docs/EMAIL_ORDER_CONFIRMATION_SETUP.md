...
The Order confirmation notification template has been updated (in `app/temp/app/tempEmailNotification.txt`) to:
...
3. **Proof link**: When `Proof PDF URL` is present, show a “Download your proof” link.
...
4. Replace the full template content with the contents of **`app/temp/app/tempEmailNotification.txt`** from this repo (copy the entire file and paste into the Shopify editor).
5. Save.
