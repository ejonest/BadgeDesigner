...
import { getSignDesignForUserShop } from "~/utils/supabase";
import { parseOr400, savedDesignDetailQuerySchema } from "~/utils/validation";

/** GET /api/saved-sign-design-detail?shop=&userId=&designId= */
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    const row = await getSignDesignForUserShop(userId, shopId, designId);
    if (!row?.design_data) {
    ...
    console.error("[api.saved-sign-design-detail] error:", error);
    return json(
      {
