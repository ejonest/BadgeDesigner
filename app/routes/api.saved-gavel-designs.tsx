...
import { listGavelDesignGallery } from "~/utils/supabase";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    const { autosave, milestones } = await listGavelDesignGallery(
      userId,
      shopId,
    );
    return json({
    ...
    console.error("[api.saved-gavel-designs] error:", error);
    return json(
      {
