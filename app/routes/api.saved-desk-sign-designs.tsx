...
import { listDeskSignDesignGallery } from "~/utils/supabase";
import { parseOr400, savedDesignQuerySchema } from "~/utils/validation";

/** GET /api/saved-desk-sign-designs?shop=&userId= */
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    const { autosave, milestones } = await listDeskSignDesignGallery(
      userId,
      shopId,
    );
    return json({
    ...
    console.error("[api.saved-desk-sign-designs] error:", error);
    return json(
      {
