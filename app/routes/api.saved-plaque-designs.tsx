...
import { listPlaqueDesignGallery } from "~/utils/supabase";
import { parseOr400, savedDesignQuerySchema } from "~/utils/validation";

/** GET /api/saved-plaque-designs?shop=&userId= */
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    const { autosave, milestones } = await listPlaqueDesignGallery(
      userId,
      shopId,
    );
    return json({
    ...
    console.error("[api.saved-plaque-designs] error:", error);
    return json(
      {
