...
import { getPlaqueDesignForUserShop } from "~/utils/supabase";
import { parseOr400, savedDesignDetailQuerySchema } from "~/utils/validation";

/** GET /api/saved-plaque-design-detail?shop=&userId=&designId= */
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    const row = await getPlaqueDesignForUserShop(userId, shopId, designId);
    if (!row?.design_data) {
    ...
    console.error("[api.saved-plaque-design-detail] error:", error);
    return json(
      {
