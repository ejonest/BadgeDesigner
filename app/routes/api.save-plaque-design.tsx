...
/**
 * Save plaque design milestone to `plaque_designs` (manual / cart / ordered). Prunes to last 10 per user/shop.
 */
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const saved = await savePlaqueDesignMilestone(row, milestoneKind);
    ...
    console.error("[PlaqueDesigner] api.save-plaque-design error:", error);
