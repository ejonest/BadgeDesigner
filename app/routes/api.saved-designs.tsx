...
import { listBadgeDesignGallery } from "~/utils/supabase";
import { parseOr400, savedDesignQuerySchema } from "~/utils/validation";

/**
 * GET /api/saved-designs?shop=&userId=
 * Gallery list: autosave + up to 10 milestones (metadata only).
 */
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    const { autosave, milestones } = await listBadgeDesignGallery(
      userId,
      shopId,
    );
    return json({
    ...
    console.error("[api.saved-designs] error:", error);
    return json(
      {
