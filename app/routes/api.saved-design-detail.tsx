...
import { getBadgeDesignForUserShop } from "~/utils/supabase";
import { parseOr400, savedDesignDetailQuerySchema } from "~/utils/validation";

/** GET /api/saved-design-detail?shop=&userId=&designId= */
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    const row = await getBadgeDesignForUserShop(userId, shopId, designId);
    if (!row?.design_data) {
    ...
    console.error("[api.saved-design-detail] error:", error);
    return json(
      {
